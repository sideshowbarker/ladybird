...
// Length of the marked text (input-method preedit) currently inserted into the focused editable. We "fake" inline
// preedit by actually inserting the marked text into the input and then replacing it on each composition update. This
// number tells WebContent how many code units to replace before the caret.
...
- (void)insertText:(id)string replacementRange:(NSRange)replacementRange
{
    ...
    if ((!matches_current_key_event && committed.length > 0) || has_marked_text) {
        ...
        m_web_view_bridge->insert_text_from_input_method(Utf16String::from_utf8(utf8), self.marked_text_length);
...
{
    // Called by the input method as the user types each composing character. We "fake" inline preedit by actually
    // inserting the marked text into the focused editable, then replacing it on each subsequent setMarkedText or on
    // commit (insertText) or abort (unmarkText). The replace count is self.marked_text_length — the length of marked
    // text we previously inserted.
...
- (void)unmarkText
{
    // Per the NSTextInputClient contract, unmarkText finalizes (commits) the marked text. Our preedit is already
    // inserted into the focused element. So committing it is simply forgetting the marked-text length — keep the text.
    self.marked_text_length = 0;
}

- (void)inputSourceDidChange:(NSNotification*)notification
{
    // When the keyboard input source changes mid-composition, the input method may not send a commit/abort callback
    // for our pending preedit. The preedit is already inserted into the focused element, so finalize it by committing
    // in place: Keep the text, and forget the marked-text length. That keeps marked_text_length in sync —  so a later
    // composition doesn't replace the wrong characters.
    (void)notification;
    self.marked_text_length = 0;
}
