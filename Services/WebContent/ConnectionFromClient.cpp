    ...
    i32 offset_start, i32 offset_end, String text)
{
    ...
    if (action == "set_caret_offset"sv || action == "set_selection"sv) {
        // Both work via DOM Selection. offset_start/offset_end are Unicode character offsets into the node's text
        // (or the node's first text child, if the node is an element).
        ...
        // Convert character offsets to UTF-16 code unit offsets for DOM Selection.
        ...
        // Range-aware scrolling is not exposed on DOM::Element; scroll the node into view as a fallback.
        if (auto* element = as_if<Web::DOM::Element>(*node))
            (void)element->scroll_into_view();
    } else if (action == "set_selected"sv) {
        // offset_start carries the desired selection state (1 = select, 0 = deselect). Used for <option> and
        ...
        if (doc) {
            ...
            Function<void(i64, int)> dump_node = [&](i64 id, int depth) {
                ...
                if (!node->url.is_empty()) {
                    ...
                    builder.append(node->url.bytes_as_string_view());
                    builder.append("\""sv);
                }
