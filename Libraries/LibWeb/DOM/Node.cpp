...
void Node::build_accessibility_tree(AccessibilityTreeNode& parent)
{
    ...
    if (is_document()) {
        ...
        if (element->include_in_accessibility_tree()) {
            ...
            // HTMLSelectElement renders its options inside a shadow-DOM popup; the <option> elements themselves
            // have no layout_node when the popup is closed, so the usual exclude-from-accessibility check (which
            // bails for elements without a layout_node) would skip them. Visit the options directly here, creating
