...
void AccessibilityTreeNode::serialize_tree_as_node_data(Vector<WebView::AccessibilityNodeData>& out, Document const& document, i64 parent_id) const
{
    ...
    if (value()->is_element()) {
        ...
        // <option> elements usually have no layout node (they only render inside a popup), so accessible_name
        // returns empty. Fall back to the option's label/text for these.
        ...
        if (auto const* table = as_if<HTML::HTMLTableElement>(element)) {
            ...
            // Summary: summary attribute (deprecated but sometimes present).
            ...
            // Column headers: first row's <th> cells (or any row with explicit scope="col").
            // Row headers: first <th> cell in each row (or any cell with scope="row").
        ...
        if (auto const* progress = as_if<HTML::HTMLProgressElement>(element)) {
            ...
            if (input->value_as_number_applies()) {
                ...
                // Minimum/maximum/step can be read from the element's attributes if set.
        ...
        // Checked state: HTML <input type=checkbox|radio> and role="checkbox"/"radio" with aria-checked.
        ...
        // Expanded state: <details> open attribute, or aria-expanded.
        ...
        // Multi-selectable: <select multiple> or aria-multiselectable="true".
        ...
        // Pressed (toggle button) from aria-pressed.
        ...
        if (auto const* anchor = as_if<HTML::HTMLAnchorElement>(element)) {
            (void)anchor;
            // TODO: Expose visited state. For now, treat all anchors with href as not-visited since LibWeb
            // doesn't expose a simple "has this URL been visited" API on the element.
        }

        // Keyboard shortcut from accesskey attribute.
        ...
        if (auto parent = text_node.parent_element()) {
            ...
            // formatting changes (e.g. <p>Hello <b>world</b></p>, where "Hello " and "world" are separate text
            // leaves with different computed_properties).
            if (auto properties = parent->computed_properties()) {
                ...
                // Font style: italic/oblique/normal. font_slope() returns an Open-Type style value; 0 is upright,
                // any positive value indicates italic/oblique. We distinguish the keyword explicitly by asking the
                // underlying FontStyleStyleValue for its keyword.
                ...
                // Text decoration: underline / line-through / overline, joined by spaces for multiple values.
                ...
                // Layout::TextNode with PaintableFragments, we use the fragment data for correct wrapping.
                // Otherwise fall back to a single-line font-metrics computation.
                auto utf8 = node_data.name.bytes_as_string_view();
                Utf8View view { utf8 };

                auto* layout = text_node.layout_node();
                if (auto paintable_with_lines = as_if<Painting::PaintableWithLines>(layout ? layout->first_paintable() : nullptr);
                    paintable_with_lines && !paintable_with_lines->fragments().is_empty()) {
                    // Use Layout fragments: each fragment corresponds to one line of this text node.
                    auto const& fragments = paintable_with_lines->fragments();
                    auto base_offset = fragments[0].offset();
                    auto text_utf16 = text_node.data().utf16_view();
                    for (auto const& fragment : fragments) {
                        auto line_top_y = static_cast<i32>((fragment.offset().y() - base_offset.y()).to_int());
                        auto line_height = static_cast<i32>(fragment.size().height().to_int());
                        ...
                        float cumulative_x = static_cast<float>((fragment.offset().x() - base_offset.x()).to_int());
    ...
    if (auto selection = document.get_selection()) {
        ...
        if (focus_node && focus_node == value().ptr()) {
            // DOM offsets into text nodes are UTF-16 code unit offsets. AT-SPI2 expects Unicode character offsets.
            // They match for ASCII and BMP characters but diverge for astral-plane codepoints (emoji etc.), where
            // one codepoint occupies two UTF-16 code units. Convert via Utf16View::code_point_offset_of.
