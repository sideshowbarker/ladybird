...
struct WEBVIEW_API AccessibilityNodeData {
    ...
    // For AtkTable: IDs of the caption and summary elements (non-table nodes), and per-column/per-row header
    // cell IDs. Empty / -1 when not applicable.
    ...
    // Tri-state expansion: NotApplicable for non-expandable elements, Collapsed/Expanded for <details>,
    ...
    bool is_multi_line { false };       // <textarea> or contenteditable block
    ...
    // Populated from the DOM Selection for the node containing the selection focus (or the collapsed-selection
    // caret, i.e. no text selected and cursor positioned here).
    i32 caret_offset { -1 };

    // Text selection range within this node's text, as Unicode character offsets. Both are -1 if there is no
    // selection in this node. When set, [selection_start, selection_end) describes the selected range.
    i32 selection_start { -1 };
    i32 selection_end { -1 };

    // Text formatting attributes for text leaf nodes, read from the parent element's computed style. Stored as
    // strings in the format expected by AT-SPI2. Empty on non-text-leaf nodes.
    String font_family;
    ...
    // For text leaf nodes, per-character (x, y) offsets in CSS pixels, relative to the text's left edge and the
    // top of the text's first line. Length equals the character count; entry N is the top-left position where
    // character N starts. Populated from Layout's PaintableFragments when available, which gives correct
    // positions for wrapping text; falls back to single-line font-metrics approximation otherwise.
    ...
    // Per-line height in CSS pixels (index matches line_break_character_offsets). Used to compute vertical extent
    // of per-character rectangles for wrapping text.
