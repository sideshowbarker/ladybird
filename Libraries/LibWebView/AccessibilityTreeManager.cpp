...
{
    // Detect live region content changes before replacing the tree. Needs an index of the new nodes by
    // id so the ancestor walk below is O(depth) rather than O(N) per step — without this, a full
    // traversal of an N-node tree would be O(N²) on every tree update.
