...
class _ReaderArrivesAfterLoad(unittest.TestCase):
    ...
    Which browser element holds the keyboard focus after launch depends on the environment: the web view, which opening
    a URL focuses, or the address bar, which takes the focus back a moment later on some machines. So no test here asks
    the application for its focused element before it means to read the page: With the web view as first responder, that
    question is itself the first read into it."""
...
class ReaderArrivingAfterLoadTests(_ReaderArrivesAfterLoad):
    ...
    fetches it. No AXLoadComplete or AXFocusedUIElementChanged comes with that tree: The page doesn't pull a reader that
    turns up later away from wherever it is, and the keyboard focus stays where the browser put it."""
